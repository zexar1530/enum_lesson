...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        ...
        std::function<void()> c1 = [this](){concurRace1->DoWork(&number, ui->rb_qtConcur->isChecked(), ui->sb_initNum->value());};
        std::function<void()> c2 = [this](){concurRace1->DoWork(&number, ui->rb_qtConcur->isChecked(), ui->sb_initNum->value());};
        QtConcurrent::run(c1).then(c2);
    }
    else{
