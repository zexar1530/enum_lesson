    ...
    , ui(new Ui::MainWindow)
{
    ...
    p_lineseries = new QLineSeries;
    p_chart = new QChart;
    p_chart->legend()->hide();
    p_chartView = new QChartView;
    connect(this, &MainWindow::sig_Display, this, &MainWindow::slot_Display);
}
...
void MainWindow::on_pb_start_clicked()
{
    ...
    if (!(p_chart->series().isEmpty())) {
        p_lineseries->clear();
        p_chart->removeSeries(p_lineseries);
    }
...
{
    p_chart->setTitle("Data");
    p_chart->addSeries(p_lineseries);
    p_chart->createDefaultAxes();

    p_chartView->setChart(p_chart);
    p_chartView->show();
}
